    ...
    <Container>
      <header className="sticky top-0 flex h-16 items-center   gap-4  bg-background px-4 md:px-6 ">
              ...
              <Link
                ...
                className={
                  currentPath === item?.link
                    ? "whitespace-nowrap  text-[#016EFD]"
                    : `text-foreground transition-colors hover:text-foreground whitespace-nowrap hover:text-[#016EFD]  `
                }
              >
                {item?.title}
              </Link>
        ...
        <div className="flex w-full items-center justify-end gap-4 md:ml-auto md:gap-2 lg:gap-4">
          <Link
            href="/AKM_SYFUL_ALOM_Resume.pdf"
            target="_blank"
            download={true}
          >
            <Button variant={"outline"} className="font-bold ">
              <ArrowBigDownDash className="shrink-0  mr-0.5" /> Get Resume
            </Button>
          </Link>
